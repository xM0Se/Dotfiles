...
CASE=$(/usr/sbin/system_profiler SPBluetoothDataType | /usr/bin/grep -A6 "Moritz’s AirPods:" | tail -n +5 | head -n 1 | /usr/bin/grep -oE '[0-9]+')

LEFT=$(/usr/sbin/system_profiler SPBluetoothDataType | /usr/bin/grep -A6 "Moritz’s AirPods:" | tail -n +6 | head -n 1 | /usr/bin/grep -oE '[0-9]+')

RIGHT=$(/usr/sbin/system_profiler SPBluetoothDataType | /usr/bin/grep -A6 "Moritz’s AirPods:" | tail -n +7 | head -n 1 | /usr/bin/grep -oE '[0-9]+')
